      ...
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        return tx.attendance.create({
          data: {
            ...
            date: todayDate,
            checkInTime: new Date(),
            checkInLatitude: dto.latitude?.toString(),
            checkInLongitude: dto.longitude?.toString(),
            checkInMethod: 'PWA',
            status: 'PRESENT',
            notes: dto.notes,
          },
    ...
    try {
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        return tx.attendance.update({
          ...
          data: {
            checkOutTime: new Date(),
            checkOutLatitude: dto.latitude?.toString(),
            checkOutLongitude: dto.longitude?.toString(),
            checkOutMethod: 'PWA',
  ...
  async syncOfflineEvent(tenantId: string, userId: string, body: any) {
    ...
    const created = await this.prisma.attendanceLog.create({
      data: {
        ...
        syncedAt: now,
        wasOfflineSync: diffMs > 6 * 60 * 1000,
        source: AttendanceSource.WEB_PWA,
        isAnomaly,
        anomalyReason,
      },
