...
export default function AuditLogsPage() {
  ...
  const fetchLogs = async (pageNum: number) => {
    try {
      setLoading(true);
      const res = await apiRequest<any>(`/settings/audit-logs?page=${pageNum}&limit=50`);
      setLogs(res.data);
  ...
  return (
    <div className="space-y-6 max-w-6xl pb-10">
      <div>
        ...
        <p className="text-sm text-[var(--text-muted)] mt-1">Immutable record of critical administrative actions.</p>
      </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {log.user ? (
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-[var(--text-primary)]">{log.user.firstName} {log.user.lastName}</span>
                          <span className="text-xs text-[var(--text-muted)]">{log.user.email}</span>
                        </div>
                      ) : (
